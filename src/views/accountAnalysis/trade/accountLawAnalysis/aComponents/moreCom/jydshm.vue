...
export default {
  ...
  methods: {
    ...
    async getTb() {
      //获取图表数据
      let { data: res } = await this.$http.post(
        `/funddetailh/getPersonalFundDetailhCount`,
        {},
        {
          params: {
      ...
      if (res.code === 1) return this.$message.error(res.msg);
      if (res.code === 0) {
        this.echart2Data.data = res.data.inmoney_detailhs;
        this.echart3Data.data = res.data.outmoney_detailhs;
        this.echart1Data.data = res.data.summoney_detailhs;
        this.show = true;
      }
    },
    async getList() {
      //获取表格数据
      let { data: res } = await this.$http.post(
        `/funddetailh/getPersonalFundDetailhDetails`,
        {
          name: this.$route.query.name,
          kzname: this.currentKzId,
